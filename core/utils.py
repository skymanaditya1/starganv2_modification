...
# This performs the image translation on the source image x 
# using the style from the style vector of the reference image
@torch.no_grad()
def translate_using_reference(nets, args, x_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size() # x_src.shape -> N x 3 x 256 x 256
    wb = torch.ones(1, C, H, W).to(x_src.device)
    x_src_with_wb = torch.cat([wb, x_src], dim=0)

    masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None
    s_ref = nets.style_encoder(x_ref, y_ref) # 16 x 64 (16 images and 64 dimensional style vector)
    s_ref_list = s_ref.unsqueeze(1).repeat(1, N, 1) # 16 x N x 64
    x_concat = [x_src_with_wb] # N+1 x C x H x W
    # The 16, 64 dimensional style vector needs to be applied to all the src images 
